package com.example.finalmerge.POStRequirementsQuiz.outStreamQuiz.outStream_CS.outStream_CS_minor;
...
public class outStreamCSminorQuiz extends AppCompatActivity implements View.OnClickListener{
    ...
    public void onClick(View view) {
        ...
        if(clickedButton.getId()==R.id.submit_btn){
            ...
            if(!selectedAnswer.equals(outStreamCSminorQuiz_QuestionAnswer.correctAnswers[currentQuestionIndex])){
                finishQuiz_fail();
                return;
            }
            currentQuestionIndex++;
            if (currentQuestionIndex < outStreamCSminorQuiz_QuestionAnswer.question.length) {
                loadNewQuestion();
            }
            else {
...
//            clickedButton.setBackgroundTintList(ColorStateList.valueOf(Color.BLUE));
    ...
    void loadNewQuestion(){
        ...
        questionTextView.setText(outStreamCSminorQuiz_QuestionAnswer.question[currentQuestionIndex]);
        ans_A.setText(outStreamCSminorQuiz_QuestionAnswer.choices[currentQuestionIndex][0]);
        ans_B.setText(outStreamCSminorQuiz_QuestionAnswer.choices[currentQuestionIndex][1]);
    }
